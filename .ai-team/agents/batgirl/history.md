...
📌 Bug fix (2026-02-14): Track routing — crossed rails near village plaza. Root cause: `GetPlatformOffset()` shifted village station Z by up to `slot * 8` blocks away from the VillageGenerator's expected position at `cz + 17`, AND the Z-first L-shaped path placed the rail corner near the village plaza. Fix: (1) Removed `GetPlatformOffset` entirely — hub-and-spoke means each village has exactly one track, no collision avoidance needed. (2) Changed L-shaped path from Z-first (corner at `(srcX, dstZ)`) to X-first (corner at `(dstX, srcZ)`) so the track exits the village heading east/west and turns far from the plaza. Crossroads radial slot system unchanged.
📌 Learning (2026-02-14): L-shaped rail paths should go X-first from village stations (which face north-south) — this pushes the corner turn far from both the village plaza and the crossroads, avoiding crossed rails near stations. Z-first creates the corner right at the village edge.

📌 Radial Crossroads Station Orientation complete (2026-02-14): Crossroads station platforms now orient radially based on their angular position around the hub. Stations primarily east/west of center run east-west (track along X axis); stations primarily north/south run north-south (track along Z axis). `StationOrientation` enum added to `Bridge.Data/WorldConstants.cs`. `GetCrossroadsSlotPosition` returns orientation alongside coordinates. `GenerateStationPlatformAsync` accepts orientation and swaps all coordinate calculations (foundation, rails, walkways, shelter, signs, dispenser, decorations). Track path L-direction chosen so final segment matches destination station orientation. CrossroadsGenerator placeholder "Slot N" signs removed, replaced with cobblestone walkways from plaza edge (radius 30) to station ring (radius 35). TrackGenerator adds cobblestone walkway from each Crossroads station back toward plaza.
📌 Learning (2026-02-14): Crossroads station orientation must match the radial direction from hub center — use |cos(angle)| vs |sin(angle)| to determine EastWest vs NorthSouth. The L-path approach direction must align with the destination station's orientation: E-W station needs X as the final approach axis (Z-first path), N-S station needs Z as the final approach axis (X-first path).
📌 Learning (2026-02-14): CrossroadsGenerator should NOT place station infrastructure (signs, platforms) that TrackGenerator will overwrite — it leads to misaligned duplicates. CrossroadsGenerator lays walkway pathways only; TrackGenerator builds the actual stations.
